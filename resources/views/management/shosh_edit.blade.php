								...
								<div class="col-md-4">
											 <p style="font-weight: bold;font-size: 15px;">Sale Amount</p>
											    <div class="input-group mb-3">
                                                       ...
                                                       <input class="form-control"  value="{{$management->sale_amount}}" aria-label="Amount (to the nearest dollar)" name="sale_amount" type="number" placeholder="Sale Amount"  step="any">
                                                   </div>
                                                 </div>
									...
									<select name="month" id="" class="form-control">
										@for($i=1;$i<=12;$i++)
											@if($i == 1)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >January</option>
											@elseif($i == 2)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >February</option>
											@elseif($i == 3)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >March</option>
											@elseif($i == 4)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >April</option>
											@elseif($i == 5)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >May</option>
											@elseif($i == 6)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >June</option>
											@elseif($i == 7)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >July</option>
											@elseif($i == 8)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >August</option>
											@elseif($i == 9)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >September</option>
											@elseif($i == 10)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >October</option>
											@elseif($i == 11)
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >November</option>
											@else
												<option value="{{ $i }}" {{ $management->month == $i ? 'selected' : '' }} >December</option>
											@endif
										@endfor
									</select>
